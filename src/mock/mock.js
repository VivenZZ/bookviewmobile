...
var listData = function(num) {
    ...
    for (let i = 0; i < num; i++) {
        let newList = { // 详细 规则 参照mockjs官网
            title: Random.cword(1, 6), // 书名
            author_name: Random.cname(), // 作者 Random.cname() 随机生成中文名
            imgSrc: Random.dataImage('132x176', '这是图片中的文本'), // 书籍图片 Random.dataImage( size, text ) 生成图片（base64位数据格式）
